...
class FraudDetector:
    ...
    def isFraud(self):
        ...
        pipeline = 'pipeline'
        for sklearn_classifier in sklearn_classifiers:
            classifier_sklearn = classifiers.get(sklearn_classifier)
            y_pred = classifier_sklearn.predict(X)
            output[mapping.get(sklearn_classifier)] = y_pred

        for tf_classifier in tensorflow_classifiers:
            classifier_ann = classifiers.get(tf_classifier)
            y_pred = classifier_ann.predict(X).astype(int).reshape(1, -1)[0]
            output[mapping.get(tf_classifier)] = y_pred
